...
const ArchivedReceiptImg = require("./models/ArchivedReceiptImg");
// 设置 Multer 中间件
...
module.exports = function (app) {
  ...
  app.get(
    ...
    billController.getInvoicesWithCondition,
  );
  ...
  // 自有车
  app.get("/build_invoice_self", billController.getBuildInvoiceSelf);
  app.get("/settle_bill_self", billController.getSettleBillSelf);
  app.get("/settle_ticket_self", billController.getSettleTicketSelf);
  app.get("/settle_money_self", billController.getSettleMoneySelf);
  app.get("/settle_vessel_self", billController.getSettleVesselSelf); // new added
  ...
  app.post(
    "/collection_not_require_settle",
    billController.postSettleCollectionNotRequire,
  );
  ...
  app.post("/settle_upload_receipt", billController.postSettleUploadReceipt);
  ...
  app.post(
    "/datamgt/warehouse_modify",
    nlDataController.httpPostWarehouseModify,
  );
  app.post(
    "/datamgt/warehouse_delete",
    nlDataController.httpPostWarehouseDelete,
  );
  ...
  app.post(
    "/datamgt/destination_modify",
    nlDataController.httpPostDestinationModify,
  );
  app.post(
    "/datamgt/destination_delete",
    nlDataController.httpPostDestinationDelete,
  );
  ...
  app.get(
    "/get_vessel_alloc_detail",
    queryController.getVesselAllocationDetail,
  );
  // 客户营业额统计
  ...
  //app.get('/get_invoices_bill', queryController.getInvoiceBill1);

  app.post("/upload-receipt-img", upload.single("image"), async (req, res) => {
    try {
      const inv_no = req.body.inv_no;
      const image = await ReceiptImg.findOne({ inv_no: inv_no });
      if (image) {
        // update
  ...
  app.get("/get-receipt-img", async (req, res) => {
    try {
      const inv_no = req.query.q;
      const image = await ReceiptImg.findOne({ inv_no: inv_no });
      if (image) {
        res.json({
          ok: true,
        ...
        const img = await ArchivedReceiptImg.findOne({ inv_no: inv_no });
        if (img) {
          res.json({
            ok: true,
          ...
          res.json({ ok: false, data: "Image not found" });
        }
  ...
  app.post("/post-carrier-department", async (req, res) => {
    try {
      const data = req.body;
      const veh = await Vehicle.findOne({ name: data.vehName });
      if (veh) {
        let found = false;
        veh.real_boss.forEach((elem) => {
          if (elem.waybill_no === data.wno) {
            elem.rb = data.boss;
            found = true;
          }
        });
        ...
        if (!found) {
          veh.real_boss.push({ waybill_no: data.wno, rb: data.boss });
        }
