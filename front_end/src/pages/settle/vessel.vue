...
async function handleSettleFromBasket(items: any[]) {
  ...
  // 验证船运单下的所有车是否都已处理
  const vesselItems = items.filter(item => item.isVessel)
  for (const vesselItem of vesselItems) {
    // 查找该船运单的原始数据
    const originalInv = dbRecords.value.find(inv => inv.waybill_no === vesselItem.waybill_no)
    if (originalInv && originalInv.veh && originalInv.veh.length > 0) {
      // 检查所有车是否都已处理（有价格或标记为不需要结算）
      const unprocessedVehicles = originalInv.veh.filter((veh: any) => {
        const price = veh.veh_price || 0
        return price === 0 // 未输入价格也未标记为不需要结算
      })

      if (unprocessedVehicles.length > 0) {
        const vehicleNames = unprocessedVehicles.map((v: any) => v.veh_name).join('、')
        toast.error(
          `船运单 ${vesselItem.waybill_no} 下还有 ${unprocessedVehicles.length} 辆车未处理：\n${vehicleNames}\n\n请先为这些车输入价格或标记为不需要结算`,
        )
        return
      }
    }
  ...
  if (itemsWithoutPrice.length > 0) {
    const confirmed = window.confirm(
      `结算篮中有 ${itemsWithoutPrice.length} 条记录还没有输入价格，是否现在输入价格？\n\n点击"确定"打开价格输入对话框`,
    )
    if (confirmed) {
      // 选中这些没有价格的记录
      tableData.value.forEach((row) => {
        row.selected = itemsWithoutPrice.some(item => item.waybill_no === row.waybill_no)
      })
      // 根据数量打开相应的价格输入对话框
