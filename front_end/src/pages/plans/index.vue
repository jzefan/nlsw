  ...
  <BasicPage title="计划列表" description="订单计划管理">
    ...
    <div v-if="showFilter" class="mb-3 p-3 border rounded-lg bg-muted/50">
      <div class="flex flex-wrap items-center gap-2">
        ...
        <SearchableCombobox v-model="filters.customerName" :search-fn="searchCompanies" placeholder="客户名称" class="w-36" />
        ...
        <UiInput v-model="filters.startDate" type="date" placeholder="开始日期" class="w-36" />
        <UiInput v-model="filters.endDate" type="date" placeholder="结束日期" class="w-36" />
    ...
    <div class="mt-4 flex items-center justify-between">
      <div class="text-sm text-muted-foreground">
        ...
        已发量 <span class="font-medium">{{ formatNumber(summary.sentWeight) }}</span> |
        未发量 <span class="font-medium">{{ formatNumber(summary.leftWeight) }}</span>
      </div>
      <div class="flex items-center gap-2">
        <UiButton
          variant="outline"
