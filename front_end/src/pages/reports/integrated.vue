...
import { Download, FileSpreadsheet, Search, RefreshCcw, X } from 'lucide-vue-next'
...
import { getIntegratedQuery, type IntegratedQueryBill } from '@/services/api/report.api'
...
const columns = computed<ColumnDef<IntegratedQueryBill>[]>(() => {
  const cols: ColumnDef<IntegratedQueryBill>[] = [
    ...
    { header: '提单状态', accessorKey: 'status' },
    ...
    for (let i = 0; i < exportBills.length; ++i) {
      ...
      if (!same && prevBill && prevBill.left_num > 0) {
        ...
        data.push([
          ...
          leftW,
          '', '', '',
          prevBill.ship_warehouse, prevBill.brand_no,
          prevBill.thickness, prevBill.width, prevBill.len, prevBill.size_type,
          prevBill.contract_no, prevBill.sales_dep,
          formatDate(prevBill.create_date), prevBill.creater,
          settleState
        ])
      }
      ...
      data.push([
        ...
        bill.inv_shipper,
        bill.inv_no,
        bill.ship_warehouse, bill.brand_no,
        bill.thickness, bill.width, bill.len, bill.size_type,
        bill.contract_no, bill.sales_dep,
        formatDate(bill.create_date), bill.creater,
        settleState
      ])
  ...
  <BasicPage title="综合查询" description="综合查询提单、运单及发货情况">
    ...
    <div class="mb-4 p-4 border rounded-lg bg-muted/30 space-y-4">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div class="hidden lg:block"></div> <!-- Spacer for Row 2, Col 4 -->
        ...
        <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 col-span-1 md:col-span-2 lg:col-span-2">
          ...
          <UiButton variant="outline" size="icon" @click="handleReset" title="重置">
            <RefreshCcw class="h-4 w-4" />
          </UiButton>
          <UiButton @click="handleQuery(true)" :disabled="loading" class="w-full sm:w-auto min-w-[100px]">
