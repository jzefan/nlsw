...
import type { VesselDetailItem, VesselRevenueData, VesselSummaryItem } from '@/services/api/vessel-statistics.api'
...
function handleDateConfirm() {
  if (dateSelectionMode.value === 'month') {
    const s = new Date(Number.parseInt(startYear.value), Number.parseInt(startMonth.value) - 1, 1)
    const e = new Date(Number.parseInt(endYear.value), Number.parseInt(endMonth.value), 1)
    if (s >= e) {
...
async function openDrillDown(type: '自有' | '外挂', mode: 'summary' | 'detail') {
  ...
  try {
    ...
    if (res.ok) {
      if (mode === 'summary') {
        summaryItems.value = res.summary_data || {}
      }
      else {
...
async function handleExport() {
  ...
  try {
    ...
    // Track column widths
    const columnWidths = Array.from({ length: 16 }).fill(0)
...
<template>
  ...
  <BasicPage v-else title="车船营业额统计" description="查看车船运输营业额及利润统计">
    ...
    <div class="mb-6 p-5 border-0 rounded-xl bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 dark:from-blue-950/30 dark:via-indigo-950/30 dark:to-purple-950/30 shadow-sm">
      <div class="flex flex-wrap items-center justify-between gap-4">
        ...
        <div class="flex flex-wrap items-center gap-4">
          <Button class="bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white shadow-md border-0" @click="showDateDialog = true">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <div class="flex gap-1 pr-2 mr-2 border-r border-gray-300 dark:border-gray-600">
            <Button variant="outline" size="sm" class="bg-white dark:bg-slate-900 shadow-sm border-0 hover:bg-gray-50" @click="openDrillDown('自有', 'summary')">
              自有统计
            </Button>
            <Button variant="outline" size="sm" class="bg-white dark:bg-slate-900 shadow-sm border-0 hover:bg-gray-50" @click="openDrillDown('自有', 'detail')">
              自有清单
            </Button>
          </div>
          <div class="flex gap-1 pr-2 mr-2 border-r border-gray-300 dark:border-gray-600">
            <Button variant="outline" size="sm" class="bg-white dark:bg-slate-900 shadow-sm border-0 hover:bg-gray-50" @click="openDrillDown('外挂', 'summary')">
              外挂统计
            </Button>
            <Button variant="outline" size="sm" class="bg-white dark:bg-slate-900 shadow-sm border-0 hover:bg-gray-50" @click="openDrillDown('外挂', 'detail')">
              外挂清单
            </Button>
          </div>
          <Button variant="outline" class="bg-white dark:bg-slate-900 shadow-sm border-0 hover:bg-gray-50" @click="handleExport">
    ...
    <div v-if="loading" class="flex flex-col items-center justify-center p-24 border-0 rounded-xl bg-gradient-to-br from-gray-50 to-white dark:from-gray-950/50 dark:to-background shadow-sm">
      <div class="relative">
        <div class="h-16 w-16 animate-spin rounded-full border-4 border-blue-200 border-t-blue-500" />
        ...
        <Table id="stat-table" class="border-collapse min-w-[1200px]">
          <TableHeader>
            <TableRow class="border-b-2 border-gray-200">
              <TableHead colspan="2" rowspan="2" class="text-center border bg-gradient-to-br from-gray-100 to-slate-100 dark:from-gray-800/50 dark:to-slate-800/50 font-bold">
                月份
              </TableHead>
              <TableHead colspan="2" class="text-center border bg-gradient-to-r from-amber-100 to-orange-100 dark:from-amber-900/30 dark:to-orange-900/30 text-amber-800 dark:text-amber-300 font-semibold">
                总营业额
              </TableHead>
              <TableHead colspan="4" class="text-center border bg-gradient-to-r from-blue-100 to-blue-200 dark:from-blue-900/30 dark:to-blue-800/30 text-blue-800 dark:text-blue-300 font-semibold">
                自有车船
              </TableHead>
              <TableHead colspan="4" class="text-center border bg-gradient-to-r from-emerald-100 to-green-100 dark:from-emerald-900/30 dark:to-green-900/30 text-emerald-800 dark:text-emerald-300 font-semibold">
                外挂车船
              </TableHead>
              <TableHead rowspan="2" class="text-center border bg-gradient-to-br from-gray-100 to-slate-100 dark:from-gray-800/50 dark:to-slate-800/50 font-bold">
                固定成本
              </TableHead>
              <TableHead rowspan="2" class="text-center border bg-gradient-to-br from-gray-100 to-slate-100 dark:from-gray-800/50 dark:to-slate-800/50 font-bold">
                短驳应收
              ...
              <TableHead rowspan="2" class="text-center border bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 text-indigo-800 dark:text-indigo-300 font-bold">
                总利润
              </TableHead>
            </TableRow>
            <TableRow>
              <TableHead class="text-center border bg-amber-50 dark:bg-amber-900/20 text-xs font-medium">
                总吨位
              </TableHead>
              <TableHead class="text-center border bg-amber-50 dark:bg-amber-900/20 text-xs font-medium">
                总金额
              </TableHead>
              <TableHead class="text-center border bg-blue-50 dark:bg-blue-900/20 text-xs font-medium">
                吨位
              </TableHead>
              <TableHead class="text-center border bg-blue-50 dark:bg-blue-900/20 text-xs font-medium">
                应收金额
              ...
              <TableHead class="text-center border bg-emerald-50 dark:bg-emerald-900/20 text-xs font-medium">
                吨位
              </TableHead>
              <TableHead class="text-center border bg-emerald-50 dark:bg-emerald-900/20 text-xs font-medium">
                应收金额
          ...
          <TableBody>
            <template v-for="item in statisticsData" :key="item.month">
              ...
              <TableRow class="hover:bg-blue-50/30 dark:hover:bg-blue-900/10 transition-colors">
                <TableCell rowspan="2" class="text-center border font-medium align-middle bg-gray-50/50 dark:bg-gray-800/30">
                  {{ item.month }}
                </TableCell>
                <TableCell class="text-center border bg-indigo-50/50 dark:bg-indigo-900/20 text-xs font-bold text-indigo-700 dark:text-indigo-400">
                  船运
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(item.vsTotal) }}
                </TableCell>
                <TableCell class="text-right border font-bold text-orange-600">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(item.vsRevenue) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(item.vsOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border text-blue-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vsOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border text-red-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vsOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border font-bold">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(item.vsOwnProfit) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(item.vsNonOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border text-blue-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vsNonOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border text-red-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vsNonOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border font-bold">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(item.vsProfit) }}
                </TableCell>
                <TableCell class="text-right border text-red-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vsFixedCost) }}
                </TableCell>
                <TableCell class="text-right border">
                  -
                </TableCell>
              ...
              <TableRow class="hover:bg-amber-50/30 dark:hover:bg-amber-900/10 transition-colors">
                <TableCell class="text-center border bg-amber-50/50 dark:bg-amber-900/20 text-xs font-bold text-amber-700 dark:text-amber-400">
                  车运
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(item.vhTotal) }}
                </TableCell>
                <TableCell class="text-right border font-bold text-orange-600">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(item.vhRevenue) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(item.vhOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border text-blue-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vhOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border text-red-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vhOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border font-bold">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(item.vhOwnProfit) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(item.vhNonOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border text-blue-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vhNonOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border text-red-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vhNonOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border font-bold">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(item.vhProfit) }}
                </TableCell>
                <TableCell class="text-right border text-red-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.vhFixedCost) }}
                </TableCell>
                <TableCell class="text-right border text-blue-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.drayage) }}
                </TableCell>
                <TableCell class="text-right border text-blue-600">
                  <span class="text-xs mr-0.5">¥</span>{{ formatNum(item.forklift) }}
                </TableCell>
            ...
            <template v-if="summaryTotals">
              <TableRow class="bg-gradient-to-r from-slate-100 to-gray-100 dark:from-slate-800/50 dark:to-gray-800/50 font-bold">
                ...
                <TableCell class="text-center border text-xs text-indigo-700 dark:text-indigo-400">
                  船运
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(summaryTotals.vsTotal) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsRevenue) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(summaryTotals.vsOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsOwnProfit) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(summaryTotals.vsNonOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsNonOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsNonOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsProfit) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vsFixedCost) }}
                </TableCell>
                <TableCell class="text-right border">
                  -
                </TableCell>
              ...
              <TableRow class="bg-gradient-to-r from-slate-100 to-gray-100 dark:from-slate-800/50 dark:to-gray-800/50 font-bold">
                <TableCell class="text-center border text-xs text-amber-700 dark:text-amber-400">
                  车运
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(summaryTotals.vhTotal) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhRevenue) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(summaryTotals.vhOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhOwnProfit) }}
                </TableCell>
                <TableCell class="text-right border">
                  {{ formatNum(summaryTotals.vhNonOwnWeight) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhNonOwnIncome) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhNonOwnDeposit) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhProfit) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.vhFixedCost) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.drayage) }}
                </TableCell>
                <TableCell class="text-right border">
                  <span class="text-xs mr-0.5 font-normal">¥</span>{{ formatNum(summaryTotals.forklift) }}
                </TableCell>
    ...
    <DialogContent class="sm:max-w-[450px] border-0 shadow-xl">
      ...
      <div class="grid gap-4 py-4">
        <div class="flex items-center justify-center gap-2 p-1 bg-muted/50 rounded-lg">
          <Button :variant="dateSelectionMode === 'month' ? 'default' : 'ghost'" size="sm" class="flex-1" :class="dateSelectionMode === 'month' ? 'shadow-sm' : ''" @click="dateSelectionMode = 'month'">
            按月
          </Button>
          <Button :variant="dateSelectionMode === 'year' ? 'default' : 'ghost'" size="sm" class="flex-1" :class="dateSelectionMode === 'year' ? 'shadow-sm' : ''" @click="dateSelectionMode = 'year'">
            按年
          </Button>
          <Button :variant="dateSelectionMode === 'custom' ? 'default' : 'ghost'" size="sm" class="flex-1" :class="dateSelectionMode === 'custom' ? 'shadow-sm' : ''" @click="dateSelectionMode = 'custom'">
            自定义
          </Button>
        </div>
          ...
          <div class="grid gap-2">
            ...
            <div class="grid grid-cols-2 gap-2">
              <Select v-model="startYear">
                <SelectTrigger><SelectValue placeholder="年份" /></SelectTrigger><SelectContent>
              ...
              <Select v-model="startMonth">
                <SelectTrigger><SelectValue placeholder="月份" /></SelectTrigger><SelectContent>
        ...
        <div v-else class="grid gap-4">
          <div class="grid gap-2">
            <Label>开始日期</Label><DatePicker v-model="formattedStartDate" :disabled-date="disableStartDate" disabled-hint="开始日期不能晚于结束日期" />
          </div>
          <div class="grid gap-2">
            <Label>结束日期</Label><DatePicker v-model="formattedEndDate" :disabled-date="disableEndDate" disabled-hint="结束日期不能早于开始日期" />
          </div>
        </div>
      </div>
      <DialogFooter>
  ...
  <Dialog v-model:open="showSummaryDialog">
    <DialogContent class="min-w-[60vw] flex flex-col max-h-[80vh] border-0 shadow-xl">
      ...
      <DialogFooter class="mt-2">
        <Button variant="outline" class="shadow-sm" @click="() => {}">
          <Download class="w-4 h-4 mr-2" />
          导出Excel
        </Button>
        <Button @click="showSummaryDialog = false">
          关闭
        </Button>
      </DialogFooter>
    </DialogContent>
