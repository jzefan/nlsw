...
function handleBack() {
  ...
  router.push('/data-process/round-steel')
}
...
<template>
  <BasicPage title="圆钢数据处理" description="上传ERP圆钢数据，按装车单号分组编辑，导出双Sheet Excel">
    <template #actions>
      <UiButton variant="outline" size="sm" @click="handleBack">
    ...
    <div v-else-if="currentStep === 4" class="flex flex-col gap-4">
      ...
      <div v-if="savedBatchId" class="p-3 border border-green-300 rounded-lg bg-green-50 dark:bg-green-950/30">
        ...
        <UiButton variant="outline" size="sm" class="mt-2" @click="router.push('/data-process/round-steel')">
          <ArrowLeft class="w-4 h-4 mr-1" />
          返回列表
        </UiButton>
      </div>
    ...
    <ConfirmDialog
      ...
      confirm-button-text="确定返回"
      cancel-button-text="取消"
      @confirm="confirmBack"
    >
      <template #title>确认返回</template>
      <template #description>当前有未完成的数据处理，返回将丢失已操作的数据。</template>
    </ConfirmDialog>
  </BasicPage>
</template>
