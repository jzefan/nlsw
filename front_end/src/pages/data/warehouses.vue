...
import { addWarehouse, deleteWarehouse, getWarehouses, updateWarehouse, type DataDictItem } from '@/services/api/data-dict.api'
...
async function loadData() {
  loading.value = true
  try {
    const res = await getWarehouses({ page: page.value, limit: limit.value, search: searchTerm.value })
    if (res.ok) {
      data.value = res.data
...
async function handleDelete(item: any) {
  if (!confirm(`确定要删除 ${item.name} 吗？`))
    return
  try {
    const result = await deleteWarehouse(item.name)
    if (result.ok) {
      toast.success('删除成功')
      loadData()
    }
