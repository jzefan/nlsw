...
exports.index = async function (req, res) {
  ...
  res.render("home", {
    ...
    scripts: [
      ...
      "js/plugins/ckeditor/lang/zh-cn.js",
  ...
  if (query_text.length > 0) {
    ...
    try {
      const bills = await Bill.find()
        .or([{ order_no: { $regex: reg } }, { bill_no: { $regex: reg } }])
        .exec();
      ...
      if (bills.length) {
        res.render("home", {
          title: "Search",
          curr_page:
            '查询提单记录里订单号或提单号中包含"' + query_text + '"的结果.',
          curr_page_name: "简单查询",
          bShowDataTable: true,
        ...
        if (invoices.length) {
          ...
          var m =
            "无数据：数据库中不存在订单号或提单号中或运单记录中运单号包含" +
            query_text +
            "的记录!";
