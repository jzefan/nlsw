...
exports.getInvoiceBill = function (req, res) {
  ...
  if (query.fType === 'invoice-first') {
    ...
    Vehicle.find({ veh_category: '自有' }).select('name').lean().exec(function (err, vehList) {
        ...
        if (bVehMode) {
          ...
          if (query.fVehMode === '自有') {
            if (Object.keys(obj).length > 0)
              obj["$and"].push({ $or: [{ vehicle_vessel_name: { $in: vehs } }, { 'bills.vehicles.veh_name': { $in: vehs } }] });
            else
              obj = { $or: [{ vehicle_vessel_name: { $in: vehs } }, { 'bills.vehicles.veh_name': { $in: vehs } }] };
          } else {
            if (Object.keys(obj).length > 0) {
              obj["$and"].push({ $and: [{ vehicle_vessel_name: { $nin: vehs } }, { 'bills.vehicles.veh_name': { $nin: vehs } }] });
            } else {
              obj = { $and: [{ vehicle_vessel_name: { $nin: vehs } }, { 'bills.vehicles.veh_name': { $nin: vehs } }] };
            }
          }
          //console.log(obj);
          Invoice.find(obj)
