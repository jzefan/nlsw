...
$(function () {
  ...
  var settleTable      = $('#table-settle-content');
  ...
  var forSelf = local_data.forSelf;
  if (forSelf) {
    btnCustomer.hide();
    btnCollection.hide();
  }
  ...
  settleTable.tablesorter({widgets:['stickyHeaders']});
    ...
    if (selected_bills.length) {
      ...
      ajaxRequestHandle('/settle_bill', 'POST',
        { settleObj: settleObj,
          ...
          selfOwned: forSelf ? 1 : 0  // new added
        }, '结算', function () {
          resetTableRow();
  ...
  function updatePriceColumn(tr, bill) {
    var price = action === "CUSTOMER" ? bill.price : bill.collection_price;
    var totPrice = price > 0 ? getStrValue(price * bill.send_weight) : '-';
    tr.find("td").eq(1).html(getState(bill));
    tr.find("td").eq(7).html(totPrice);
    tr.find("td").eq(8).html(getPriceText(price));
    //tr.popover('hide').attr('data-content', getBillTooltip(bill));
  }

  function buildPriceInputHtml(bill) {
    var str = `<div class="row form-horizontal" style="padding: 15px 15px 0"><div class="col-md-11">
...
<div class="form-group">
  <label for="{0}" class="control-label col-sm-4">{1}</label>
  <div class="input-group col-sm-8">
    <input id="{2}" type="text" name="{3}" class="form-control" value="{4}" />
...
<div class="form-group" style="margin-top: 25px">
  ...
  <div class="input-group col-sm-8">
    <input id="price-remark" type="text" name="price-remark" class="form-control" value="{5}" />
  </div>
</div></div></div>
`;
    var msg = '';
    var id = '';
    var remark = bill.incoming_price_remark ? bill.incoming_price_remark : '';
    if (action === 'COLLECTION') {
      id = bill.bill_no + '_' + bill.order;
      id = id.replaceAll('.', '-')
      msg = str.format(id, '代收代付价格', id, id, getStrValue(bill.collection_price, "", ""), remark);
    } else {
      if (bill.veh_ves_name) { // 正常有运单一定有运单车船，需要再检查？
        id = bill.inv_no;
        msg = str.format(id, bill.veh_ves_name, id, id, getStrValue(bill.price), remark);
      }
    }
  ...
  function buildPriceDialog(bill) {
    ...
    bootbox.dialog({
      message: buildPriceInputHtml(bill),
      title: "单行价格输入",
      buttons: {
    ...
    $('input[name="price-mode"]').change(function() {
      ...
      var inputVal = getPriceValue(id) //bill.bill_no + '_' + bill.order);

      if (Number(inputVal) > 0) {
        elem.val('');
        if (val === 'unit') {
          elem.val(getStrValue(inputVal / bill.send_weight));
        } else {
          elem.val(getStrValue(inputVal * bill.send_weight))
        }
      }
    ...
    function ok() {
      ...
      var updated = false;
      var price = getPriceValue(id) //bill.bill_no + '_' + bill.order);
      if (price > 0 && priceMode === 1) {
        price = price / bill.send_weight;
      }
      ...
      if (bill.incoming_price_remark !== remark) {
        updated = true;
      }
...
//    $('[data-toggle="popover"]').popover({ trigger: "hover", html: true, placement: "bottom" });
  ...
  function makeTableBodyTr(bill) {
    ...
    trHtml += `<td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td>
    ...
    return trHtml.format(getState(bill), getOrder(bill.order_no, bill.order_item_no), bill.bill_no,
      name, bill.veh_ves_name, bill.ship_to, totPrice, priceText,
      bill.send_num > 0 ? bill.send_num : '', getStrValue(bill.send_weight), bill.ship_from,
  ...
  elementEventRegister(btnExport, 'click', function() {
    var html_text = [];
    var head = '<thead><tr><th>状态</th><th>订单号</th><th>提单号</th><th>开单名称</th><th>车船</th><th>目的地</th><th>总价格</th><th>单价</th><th>发运块数</th>' +
      '<th>发运重量</th><th>起始地</th><th>发货仓库</th><th>发货日期</th><th>发货人</th><th>运单号</th><th>规格</th><th>规格大小</th><th>合同号</th></tr></thead><tbody>';
    html_text.push(head);

    var trHtml = `<tr><td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td><td>{7}</td>
    <td>{8}</td><td>{9}</td><td>{10}</td><td>{11}</td><td>{12}</td><td>{13}</td><td>{14}</td><td>{15}</td><td>{16}</td><td>{17}</td></tr>`;
    curr_show_bills.forEach(function (bill) {
      ...
      html_text.push(trHtml.format(getState(bill), getOrder(bill.order_no, bill.order_item_no), bill.bill_no,
        ...
        bill.inv_shipper ? bill.inv_shipper : '', bill.inv_no, remark, spec, bill.contract_no ? bill.contract_no : ''));
    });
  ...
  function filterData(needUpdateDbData, emptyDbData) {
    if (needUpdateDbData) {
      ...
      obj.selfOwned = forSelf ? 1 : 0;
      $.get('/get_invoices_bill', obj, function (data) {
        dbBills = [];
        if (data.ok) {
          dbBills = sortByKey(data.bills, "inv_ship_date", "DSC");
        }
...
QueryFilterD.prototype.getQueryParams = function() {
  ...
  return {
    ...
    fDate2: b ? moment(this.eDate).endOf('day').format('YYYY-MM-DD HH:mm:ss') : null,
    fType: "invoice-first"
  };
};
