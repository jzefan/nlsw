...
$(function () {
  ...
  $('input:checkbox').not('.no-icheck').iCheck({
    checkboxClass: 'icheckbox_square-green',
    radioClass: 'iradio_square-green',
    increaseArea: '20%' // optional
  });
...
function setDateGrpMinMaxRange(start, end, range = 60) {
  var edate = moment();
  var sdate = moment().subtract(range, 'months');
  end.data("DateTimePicker").setMinDate(sdate);
  end.data("DateTimePicker").setMaxDate(edate);
  start.data("DateTimePicker").setMinDate(sdate);
  start.data("DateTimePicker").setMaxDate(edate);
}
...
function ajaxRequestHandle(route, method, data, action_msg, afterDone) {
  ...
  request.done(function (data) {
    if (data.ok) {
      if (action_msg === 'no_message' || action_msg === '' || !!action_msg) {
...
function excelExport({ dataSource = [], titles = [], fileName = 'data', suffix = 'csv' }) {
  var dataType = "\uFEFF"; //解决乱码问题
...
SearchHandlerD.prototype.initial = function (route) {
  ...
  self.uiElems.bExec.on('click', function (e) {
    e.stopImmediatePropagation();
    e.stopPropagation();
