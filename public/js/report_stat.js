...
//      graph.colorField = "color";
  ...
  function showVesselDetail(isSummary, vehType, month) {
    ...
    if (isSummary) {
      ...
      $.get('/get_vessel_alloc_detail', obj, function (data) {
        if (data.ok) {
          var vObj = data.vessel_detail;
          var nameList = data.vehNameList;
  ...
  var tempResultDataForExport = [];
  function showCustomerDetail(nameList) {
    if (startDate && endDate) {
      $('body').css({'cursor': 'wait'});

      tempResultDataForExport.length = 0;
      detailTBody.empty();
      var obj = {
        fName: nameList,
      ...
      $.get('/get_customer_detail', obj, function (data) {
        if (data.ok) {
          ...
          data.detail_data.forEach(function (item) {
  ...
  elementEventRegister($('#export-detail'), 'click', function() {  
    var header = [ "订单号", "提单号", "开单名称", "车船号", "目的地", "代收价格", "客户价格", "价格", "发运块数", "发运重量", "发货日期", "运单号", "发货仓库", "规格", "牌号", "合同号" ];
    var m = moment().format('YYYY-MM-DD HH:mm');
    excelExport({ fileName: `营业额-所有明细-${m}`, suffix: 'csv', titles: header, dataSource: tempResultDataForExport })
  });
  ...
  // 设置只能查询2年以内的数据, 24 months
  setDateGrpMinMaxRange(startDateGrp, endDateGrp, 24);
