    ...
    if (currentIdx >= 0) {
      ...
      if (oper_object === 'brand' || oper_object === 'sale_dep') {
        ...
        if (oper_object === 'vehicle') {
          ...
          if (row.boss) {
            setElementValue(vehBoss, row.boss);
          } else {
            unselected(vehBoss);
          }
    ...
    if (action == 'add') {
      ...
      if (oper_object === 'brand' || oper_object === 'sale_dep') {
        ...
        if (oper_object === 'vehicle') {
          ...
          var boss = getElementValue(vehBoss);
          if (contact_name != row.contact_name || phone != row.phone || veh_type != row.veh_type || category != row.veh_category || boss !== row.boss) {
    ...
    ajaxRequestHandle(route, 'POST', data, msg, function() {
      if (ldata && currentIdx >= 0) {
        ...
        if (oper_object === 'vehicle') {
          ...
          ldata[currentIdx].boss = data.boss;
          str = '<td>{0}</td><td>{1}</td><td>{2}</td><td>{3}</td><td>{4}</td><td>{5}</td><td>{6}</td>'.format(currentIdx + 1, 
            data.name, data.veh_type, data.veh_category, data.boss, data.contact_name, data.phone);
        } else if (oper_object === 'company') {
  ...
  function resetTable(filter) {
    ...
    if (ldata.length > 0) {
      ...
      if (oper_object === 'brand' || oper_object === 'sale_dep') {
        ...
        $.each(ldata, function (idx, row) {
          if (!filter || filter === '所有记录' || row.name.indexOf(filter) >= 0) {
            dataBody.append(str.format(idx + 1, row.name, row.veh_type ? row.veh_type : '', 
            row.veh_category ? row.veh_category : '', 
            row.boss ? row.boss : '',
            row.contact_name ? row.contact_name : '', row.phone ? row.phone : ''));
          }
        });
      } else {
